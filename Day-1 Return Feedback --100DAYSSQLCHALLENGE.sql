...
/*
Namastekart, an e-commerce company, has observed a notable surge in return orders recently. They suspect that a specific group of customers may be 
responsible for a significant portion of these returns. To address this issue, their initial goal is to identify customers
who have returned more than 50% of their orders. This way, they can proactively reach out to these customers to gather feedback.

For instance, if a customer named ABC has placed 4 orders and returned 3 of them, they should be included in the output.
On the other hand, if a customer named XYZ has placed 5 orders but only returned 2, they should not be included in the output.
...
WHERE CAST  (([Return Count]*1.0/[Total Order] * 1.0)*100 AS DECIMAL(4,0))>=50.00
...
SELECT 
    Flipkart_tb.customer_name,  
    CASE 
        WHEN COUNT(Flipkart_tb.order_id) = 0 THEN NULL  -- Avoid divide by zero
        ELSE CAST(COUNT(Flipkart_Return.order_id) * 1.00 / NULLIF(COUNT(Flipkart_tb.order_id), 0) AS DECIMAL(10, 2))
    END as 'Return Percent'
FROM 
    Flipkart_tb
LEFT JOIN 
    Flipkart_Return ON Flipkart_tb.order_id = Flipkart_Return.order_id
GROUP BY 
    Flipkart_tb.customer_name
HAVING 
    COUNT(Flipkart_tb.order_id) > 0  -- Exclude customers with no orders
    AND (COUNT(Flipkart_Return.order_id) * 1.00 / NULLIF(COUNT(Flipkart_tb.order_id), 0)) >= 0.50;
