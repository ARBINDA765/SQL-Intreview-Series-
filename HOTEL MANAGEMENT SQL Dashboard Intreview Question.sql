...
WITH CTE_4 as
(
SELECT DATENAME(month,bill_date) AS 'Month-2021', item_id,sum(item_quanity) as 'Item Count'
FROM  booking_commerical
GROUP BY item_id,DATENAME(month,bill_date)
)
,CTE_4_Most_Ordered as 
(

SELECT *,DENSE_RANK() OVER (PARTITION BY [Month-2021] ORDER BY [Item Count]DESC ) 'Ranking_Most_Order' 

FROM CTE_4 
),

CTE_4_Least_Ordered as 
(SELECT * ,
DENSE_RANK() OVER (PARTITION BY [Month-2021] ORDER BY [Item Count]ASC) 'Ranking_Least_Order' FROM CTE_4 
)


--SELECT * FROM CTE_4_Sub
SELECT * FROM CTE_4_Most_Ordered  WHERE Ranking_Most_Order =1 
UNION ALL 
SELECT * FROM CTE_4_Least_Ordered  WHERE Ranking_Least_Order =1
